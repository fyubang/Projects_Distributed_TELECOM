...
public class Descente extends Observable implements Runnable  {
	...
	private void directionASuivreSiInterieur() {
		Couple gra = problemeTraite.gradientf(pointCourant);
		if(gra.norme()> seuil)
			directionASuivre = gra.mult(-1);
		else
		{
			directionASuivre =new  Couple(0,0);
			finie = true;
		}
	}
	...
	private void directionASuivreSiBord(Contrainte c) {
		Couple gra = problemeTraite.gradientf(pointCourant);
		Couple gra_contraint = c.getGradient();
		Couple c_bord = c.getBordUnitaire();
		if(gra.norme() < seuil){
			directionASuivre =new  Couple(0,0);
			finie = true;
		}
		else{
			if(gra.produitScalaire(gra_contraint)<0){	
				if( gra.estPerpendiculaire(c_bord) == true){
					directionASuivre = new Couple(0,0);
					finie = true;
				}
				else if(gra.produitScalaire(c_bord)< 0)
					directionASuivre = c_bord;
				else
					directionASuivre = c_bord.mult(-1);
			}
			else{
				directionASuivre = gra.mult(-1);
	...
	* Si la descente n'est pas finie, met la prochaine direction dans l'attribut this.directionASuivre  
	...
	private void directionASuivreSiCoin(Contrainte[] coin) {
		Couple gra = problemeTraite.gradientf(pointCourant);
		Couple grad_decompose = Couple.decompose(gra, coin[0].getGradient().mult(-1), coin[1].getGradient().mult(-1));
		if(gra.norme()< seuil){		
			directionASuivre =new  Couple(0,0);
			finie = true;
			...
			if( grad_decompose.x>= (-seuil) && grad_decompose.y >= (-seuil)){
					directionASuivre =new Couple(0,0);
					finie = true;
			}
			else if(gra.produitScalaire(coin[0].getGradient().mult(-1))<=seuil && gra.produitScalaire(coin[1].getGradient().mult(-1))<=seuil){
				directionASuivre = gra.mult(-1.0);
			}
			else{
				if(gra.produitScalaire(coin[0].getBordUnitaire())/gra.norme() <= gra.produitScalaire(coin[1].getBordUnitaire())/gra.norme()) {
					directionASuivre = coin[0].getBordUnitaire();
				}
				else {
					directionASuivre = coin[1].getBordUnitaire();
				}
	...
	public double chercheSecondPoint(Couple pointDepart, Couple dir) {
		double temp = 1.0;
		double derivee = problemeTraite.gPrime(pointDepart, dir, temp);
		while (derivee <= 0.0){
			temp = 2*temp;
			derivee = problemeTraite.gPrime(pointDepart, dir, temp);
	...
	public Couple dichotomie(Couple pointDepart, Couple dir, double t1) {	
		double t0 = 0;
		double t2 = (t0+t1)/2;
		while(!estNul(problemeTraite.gPrime(pointDepart, dir, t2)))
		{
			t2 = (t0+t1)/2;
			if(problemeTraite.gPrime(pointDepart, dir, t2)>0)
				t1 = t2;
			else
				t0 = t2;
		}
		Couple P_temp = pointDepart.ajoute(dir.mult(t2));
		return P_temp;
	}
	...
	public Couple KuhnTucker(Couple P) {
		Couple gra = problemeTraite.gradientf(P);
		Contrainte coin[] = domaine.estCoin(pointCourant);
		Couple mu = null;
		if(coin == null){
			mu = new Couple(0,0);
            if (gra.norme() > seuil){
            ...
            else if(domaine.estSurBord(P)!= null){
                double mu_x= gra.produitScalaire(domaine.estSurBord(P).getGradient());
                if (mu_x < 0){
                    return null;
                }
                else if (gra.norme() < seuil){
                	mu.x = mu_x;
                    return mu;
                }
                else if (domaine.estSurBord(P).getBordUnitaire().estPerpendiculaire(gra)==false){
                    return null;
                }else{}
			}else{}

		}else{
			Couple gra_decompose = Couple.decompose(gra, coin[0].getGradient(), coin[1].getGradient());
			if( gra_decompose.x<= -1 * seuil && gra_decompose.y <= -1 * seuil){
				return gra_decompose;			
			}
			else{
				mu = null;				
				return  mu;
