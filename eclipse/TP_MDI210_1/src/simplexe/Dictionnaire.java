...
//		incomplet = true;
	...
	public int chercherIndiceVariableSortante(int jE)  {
		...
		for(int i=1;i<=nbVarBase;i++)
		{
			if(dict[i][jE] < 0)
			{
				if((-dict[i][0])/dict[i][jE] < temp)
				{
					temp = (-dict[i][0])/dict[i][jE];
					sortante = i;
				}
			}
	...
	// Deux lignes suivantes a supprimer
		...
		double temp_iS_jE = dict[iS][jE];
		for(int j=0;j<=nbVarHorsBase;j++)
		{
			dict[iS][j] = (-dict[iS][j])/temp_iS_jE;
		}
		dict[iS][jE] = (1)/temp_iS_jE;
		for(int i=0;i<=nbVarBase;i++)
		{
			if(i != iS)
			{
				for(int j=0;j<=nbVarHorsBase;j++)
				{
					if(j!=jE)
					{
						dict[i][j] = dict[i][j]+dict[iS][j]*dict[i][jE];
					}
				}
				dict[i][jE] = dict[i][jE]*dict[iS][jE];
			}
		}
	}
	...
	public int chercherIndiceVariableEntranteAvantageuse() {	
	    ...
	    {
	        if(dict[0][j] > 0) {
	        	temp_i = chercherIndiceVariableSortante(j);
	        	if(temp_i == 0)     //si chercherIndiceVariableSortante == 0, c'est a dire il y a un problem non borne
	        	{
	        		return -1;
	        	}
	        	if((-dict[temp_i][0])*dict[0][j]/dict[temp_i][j] > temp)
	        	{
		            entrante = j;
		        	temp = (-dict[temp_i][0])*dict[0][j]/dict[temp_i][j];
	        	}
	        }
	    }
	    return entrante;
	...
	public int chercherIndiceVariableEntrantePlusPetitNumero(){		
		// Deux lignes a supprimer
		int entrante = 0;
		int temp_tab = nbVarBase+nbVarHorsBase+2;
	    for(int j=1; j <= nbVarHorsBase; j++)
	    {
	        if(dict[0][j] > 0) {
	        	if(tabVarHorsBase[j]<temp_tab)
	        	{
		            entrante = j;
		            temp_tab = tabVarHorsBase[j];
	        	}
	...
	public int chercherIndiceVariableSortantePlusPetitNumero(int jE) {		
		...
		for(int i=1;i<=nbVarBase;i++)
		{
			if(dict[i][jE] < 0)
			{
				...
				else if((-dict[i][0])/dict[i][jE] == temp)
				{
					if(tabVarBase[i]<tabVarBase[sortante])
					{
						sortante = i;
					}
				}
